  ...
  <div class="demo-page">
    <el-card shadow="hover">
      ...
      <el-table 
        ...
        :highlight-current-row="cfg.highlightCurrentRow"
        :row-class-name="cfg.rowClassName ? rowClassNameFn : undefined"
        :row-style="cfg.rowStyle ? rowStyleFn : undefined"
        :cell-class-name="cfg.cellClassName ? cellClassNameFn : undefined"
        :cell-style="cfg.cellStyle ? cellStyleFn : undefined"
        :header-row-class-name="cfg.headerRowClassName"
        :header-row-style="cfg.headerRowStyle ? headerRowStyleFn : undefined"
        :header-cell-class-name="cfg.headerCellClassName"
        :header-cell-style="cfg.headerCellStyle ? headerCellStyleFn : undefined"
        :empty-text="cfg.emptyText"
        :default-sort="cfg.defaultSort ? JSON.parse(cfg.defaultSort) : undefined"
        :tooltip-effect="cfg.tooltipEffect"
        :show-summary="cfg.showSummary"
        :sum-text="cfg.sumText"
        :summary-method="cfg.summaryMethod ? summaryMethodFn : undefined"
        :span-method="cfg.spanMethod ? spanMethodFn : undefined"
        :select-on-indeterminate="cfg.selectOnIndeterminate"
        :indent="cfg.indent"
        :lazy="cfg.lazy"
        :load="cfg.load ? loadFn : undefined"
        :tree-props="cfg.treeProps ? JSON.parse(cfg.treeProps) : undefined"
        :default-expand-all="cfg.defaultExpandAll"
        :expand-row-keys="cfg.expandRowKeys ? JSON.parse(cfg.expandRowKeys) : undefined"
        :row-key="cfg.rowKey"
        :current-row-key="cfg.currentRowKey"
        :empty-icon="emptyIconComp"
        :sort-icon="sortIconComp"
        :filter-icon="filterIconComp"
        :loading="cfg.loading"
        :element-loading-text="cfg.elementLoadingText"
        :element-loading-spinner="cfg.elementLoadingSpinner"
        :element-loading-svg="cfg.elementLoadingSvg"
        :element-loading-svg-view-box="cfg.elementLoadingSvgViewBox"
        :scrollbar-always-on="cfg.scrollbarAlwaysOn"
        :flexible="cfg.flexible"
        style="width: 100%"
        @select="onUI('select')"
        @select-all="onUI('select-all')"
        @selection-change="onUI('selection-change')"
        @cell-mouse-enter="onUI('cell-mouse-enter')"
        @cell-mouse-leave="onUI('cell-mouse-leave')"
        @cell-click="onUI('cell-click')"
        @cell-dblclick="onUI('cell-dblclick')"
        @row-click="onUI('row-click')"
        @row-contextmenu="onUI('row-contextmenu')"
        @row-dblclick="onUI('row-dblclick')"
        @header-click="onUI('header-click')"
        @header-contextmenu="onUI('header-contextmenu')"
        @sort-change="onUI('sort-change')"
        @filter-change="onUI('filter-change')"
        @current-change="onUI('current-change')"
        @header-dragend="onUI('header-dragend')"
        @expand-change="onUI('expand-change')"
      >
        ...
        <el-table-column label="操作" width="140">
          <template #default="scope">
            ...
            <el-button link type="danger" size="small" @click="remove(scope.$index)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-drawer v-model="show" title="表格属性设置" size="45%">
      <div class="drawer-content">
        ...
        <div class="section">
          ...
          <div class="form-item">
            ...
            <el-input v-model="cfg.defaultSort" placeholder='如 {"prop":"date","order":"descending"}' style="width: 200px;" />
          ...
          <div class="form-item">
            ...
            <el-input v-model="cfg.treeProps" placeholder='如 {"children":"children","hasChildren":"hasChildren"}' style="width: 200px;" />
